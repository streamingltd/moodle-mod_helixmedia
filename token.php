 ...
 * @copyright  1999 onwards Martin Dougiamas  {@link http://moodle.com}, 2025 Streaming Ltd
...
if ($ok) {
    $tool = get_config('helixmedia');

    if ($tool->ltiversion != LTI_VERSION_1P3) {
        $error = 'invalid_request';
        $ok = false;
    }

    if ($ok && $tool->clientid == $claims['sub']) {
        try {
            $ok = helixmedia_verify_jwt_signature($tool, $claims['sub'], $clientassertion);
...
if ($ok) {
    ...
    $permittedscopes = helixmedia_get_permitted_service_scopes();
...
if ($ok) {
    $token = helixmedia_new_access_token($scopes);
